...
data class NapImportResponse(
    ...
    val items: List<StatusOfRoad> = emptyList()
)

object RoadsStateApi {
    ...
    fun getAll(): List<StatusOfRoad> {
        val request = Request.Builder()
            .url("$BASE/api/road-status")
            .get()
            .withAuth()
            .build()

        val body = responseBodyOrThrow(request, "Napaka pri branju stanja cest")
        val type = object : TypeToken<List<StatusOfRoad>>() {}.type
        return gson.fromJson(body, type) ?: emptyList()
    }

    fun importFromNap(): List<StatusOfRoad> {
        val request = Request.Builder()
            .url("$BASE/api/road-status/nap/import")
    ...
    fun sync(list: List<StatusOfRoad>): Boolean {
        val body = gson.toJson(list).toRequestBody(JSON)

        val request = Request.Builder()
            .url("$BASE/api/road-status/sync")
            .post(body)
            .withAuth()
            .build()
